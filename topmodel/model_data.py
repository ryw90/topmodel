...
class ModelDataManager(object):
    ...
    def __init__(self, file_system):
        ...
        scores = [fp for fp in self.file_system.ls()
        ...
        actuals = [fp for fp in self.file_system.ls()
                   if fp.endswith(ACTUALS_FILE)]
        for path in actuals:
            basedir = os.path.split(path)[0]
            scores_bm = [p for p in self.file_system.ls(basedir)
    ...
    def search(self, target_model):
        return filter(
            lambda model: target_model in model.model_path,
            self.file_system.ls())
...
class ModelData(object):
    ...
    def get_metrics(self, bootstrap=False):
        def bootstrap_histogram(hist):
            ...
            falses = map(
                lambda x: samp(x[0] - x[1]),
                zip(hist['totals'], hist['trues']))
            return {'probs': hist['probs'],
                    'trues': trues,
                    'totals': map(lambda x: x[0] + x[1], zip(trues, falses))}

        hist = self.to_histogram_format()
        base = hmetrics.all_metrics(hist)
        if not bootstrap:
            return base
        else:
            bootstrapped = [
                hmetrics.all_metrics(bootstrap_histogram(hist))
                for x in range(bootstrap)]
            return [base] + bootstrapped

    def check_legacy_format(self):
        # have legacy data format of "score,trues,falses"
        # here we build the DataFrame to match the old scores.tsv
        # (ryw): should we just drop this? seems weird to keep
        raw = self.data_frame
    ...
    def to_histogram_format(self):
        ...
        if histogram_json is not None:
            ...
            bin_edges = map(
                lambda x: x * N_THRESHOLDS,
                range(0, THRESHOLD_BINS + 1))
            o_count = []
            f_count = []
            ...
            for i in range(THRESHOLD_BINS):
                ...
                binned = ((predicted >= bin_edges[i]) &
                          (predicted <= bin_edges[i+1]))
                o_count.append(np.count_nonzero(binned & actual))
                f_count.append(np.count_nonzero(binned))
...
class BenchmarkedModelData(ModelData):
    ...
    """
    def indexed_data_frame(self, path):
        raw = self.file_system.read_file(path)
        with io.BytesIO(raw) as f:
          df = pd.read_csv(fp_or_buffer, sep='\t', index_col=False, **kwargs)
    ...
    def to_data_frame(self, **kwargs):
        ...
        if self.data_frame is None:
            ...
            df_actuals = self.indexed_data_frame(actuals_path)
            scores_path = os.path.join(self.model_path, SCORES_BM_FILE)
            df_scores = self.indexed_data_frame(scores_path)
